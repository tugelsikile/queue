...
class Admin extends CI_Controller
{
    ...
    function delete_user()
    {
        ...
        $loket_id   = $this->uri->segment(3);
        $data_loket = $this->dbase->dataRow('user', array('user_id' => $loket_id));
        if (!$loket_id || !$data_loket) {
            die('Invalid data');
        } else {
            $data['data']   = $data_loket;
            $this->load->view('admin/form/delete_user', $data);
        }
    }
    function delete_user_submit()
    {
        ...
        $json['t'] = 0;
        $loket_id   = $this->input->post('user_id');
        $data_loket = $this->dbase->dataRow('user', array('user_id' => $loket_id));
        if (!$loket_id || !$data_loket) {
            $json['msg'] = 'Invalid data';
        } else {
            $this->dbase->dataUpdate('user', array('user_id' => $loket_id), array('user_status' => 0));
            //$this->dbase->dataUpdate('poli', array('loket_id' => $loket_id, 'poli_status' => 1), array('loket_id' => NULL));
            $json['t'] = 1;
            $json['msg'] = '<strong>Data User</strong> berhasil dihapus';
        }
        die(json_encode($json));
    }

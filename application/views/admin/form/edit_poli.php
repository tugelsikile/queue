<form id="new_form" class="form form-horizontal">
    <input type="hidden" name="poli_id" value="<?php echo $data->poli_id;?>">
    <div class="poli_name form-group">
        <label for="poli_name" class="control-label col-md-3">Nama Poli</label>
        <div class="col-md-9">
            <input type="text" name="poli_name" id="poli_name" class="form-control" placeholder="Nama Poli" value="<?php echo $data->poli_name;?>">
        </div>
    </div>
    <div class="poli_kode form-group">
        <label for="poli_kode" class="control-label col-md-3">Kode Poli</label>
        <div class="col-md-9">
            <input type="text" name="poli_kode" size="1" id="poli_kode" class="form-control" placeholder="Kode Poli (MAX 1 karakter)" value="<?php echo $data->poli_kode;?>">
        </div>
    </div>
...
<script>
    <?php
            if (strlen(trim($data->loket_id)) > 0){
                echo "$('#loket_id').val('".$data->loket_id."');";
            }
    ?>
    ...
    $('#modal1 #new_form').submit(function (e) {
        ...
        $.ajax({
            url     : '<?= site_url()?>/admin/edit_poli_submit',
            type    : 'POST',
            dataType: 'JSON',
            contentType: false,
            processData: false,

<form id="new_form" class="form form-horizontal" style="text-align: center">
    ...
    Anda yakin ingin menghapus data user <strong><?php echo $data->user_fullname; ?></strong> ?
</form>
...
<script>
    ...
    $('#modalConfirm #new_form').submit(function(e) {
        $('#modalConfirm .btn-submit').html('<i class="fa fa-spin fa-refresh"></i> Ya').addClass('disabled').prop({
            'disabled': true
        });
        $.ajax({
            url: '<?= site_url() ?>/admin/delete_user_submit',
            type: 'POST',
            dataType: 'JSON',
            data: $(this).serialize(),
            success: function(dt) {
                if (dt.t == 0) {
                    show_info(dt.msg);
                    $('#modalConfirm .btn-submit').html('<i class="fa fa-check"></i> Ya').removeClass('disabled').prop({
                        'disabled': false
                    });
                } else {

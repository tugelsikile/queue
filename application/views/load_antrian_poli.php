...
if ($data){
    ...
    foreach ($data as $val){
        if ($val->is_called == 0){
            ...
            $call = '<a title="Ulangi panggilan" href="javascript:;" class="btn btn-xs btn-default" onclick="call_antri(this);return false" data-id="'.$val->qpol_id.'">
                        <i class="fa fa-microphone"></i>
                    </a>
                    <span class="label label-primary">Terpanggil</span> ';
        }
        ?>
        <tr class="row_<?php echo $val->qpol_id;?>">
            <td align="center"><?php echo $val->qpol_kode1.$val->qpol_kode2;?></td>
            <td align="center"><?php echo date('H:i',strtotime($val->qpol_date));?></td>
            <td align="center" class="call_<?php echo $val->qpol_id;?>"><?php echo $call;?></td>
        </tr>
        <?php
    }
}
